...
public abstract class BaseBetweenFilter<D, F, M extends Comparable<? super M>, V> extends BaseFilter<D, F, M, List<V>> {
	...
	public BaseBetweenFilter(SingularAttributeGetter<D, M> field, Function<F, List<V>> getter, Function<V, M> toDb) {
		super(field, getter);
		this.toDb = toDb;
	}

	public BaseBetweenFilter(SingularAttributeGetter<D, M> field, Function<F, List<V>> getter) {
