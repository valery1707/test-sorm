...
public class IpFilter<D, F> extends BaseFilter<D, F, String> {
	public IpFilter(String field, Function<F, String> getter) {
		super(field, getter);
	}
	...
	protected Predicate toPredicateImpl(Root<D> root, CriteriaQuery<?> query, CriteriaBuilder cb, @Nonnull String filter) {
		...
		if (subnet != null) {
			return cb.between(root.get(field()), ipToNumber(subnet.getInfo().getLowAddress()), ipToNumber(subnet.getInfo().getHighAddress()));
		}
		return null;
	}
}
